    ...
    for word in words:
        ...
        for single in whole_word:
            if single not in characters:
                ...
                if os.path.exists(f"./hsk6/{single}.png"):
                    charDownloadCount += 1
                    print(f"Exists: {single} -> {charDownloadCount}")
                if os.path.exists(f"./hsk6/sheets/{single}-sheet.png"):
                    sheetDownloadCount += 1
                    print(f"Sheet Exists: {single} -> {sheetDownloadCount}")
                else:
                    ...
                    if div_char is not None:
                        ...
                        if img is not None:
                            ...
                            if os.path.exists(f"./hsk6/{single}.png"):
                                print(f"File already exists for character: {single}")
                            else:
                                response = requests.get(img_url)
                                if response.status_code == 200:
                                    with open(f"./hsk6/{single}.png", "wb") as f:
                                        f.write(response.content)
                                    charDownloadCount += 1
                                    print("charDownloaded: " + str(charDownloadCount))
                                else:
                                    print(
                                        f"Failed to download image for character: {single}"
                                    )
                            response = requests.get(img_url)
                            if response.status_code == 200:
                                with open(f"./hsk6/{single}.png", "wb") as f:
                    ...
                    if div_sheet is not None:
                        ...
                        if img is not None:
                            ...
                            if response.status_code == 200:
                                with open(
                                    f"./hsk6/sheets/{single}-sheet.png", "wb"
                                ) as f:
                                    f.write(response.content)
                                urls.append(img_url)
                                sheetDownloadCount += 1
